...
the companion app's approved *Multi-Source Library* + *Project Model* (see the companion app's
multi-source-library and project-model design docs), and let okashi **create and
...
**okashi may write any project's `manifest.json`**, including the companion app's shared corpus. The
...
- **Confirm (consequential):** **structural** edits — reorder / insert / remove / move — prompt
  a confirmation ("Move chapter 3 to the end? Rewrites manifest.json"). This mirrors the companion
  app's own confirm sheet (multi-source §6). Structural edits are the "structuring mode" ([[roadmap-
...
- **HARD-GATE (both repos):** this reverses the companion app's "okashi never writes manifests; the only
  writers are the companion app's `ManuscriptStore`" line. Reflect the new authority in **both** — okashi
  `CLAUDE.md` §1 and the companion app's `SPEC.md` / multi-source §4 / project-model §4. **No companion-app code
  change expected** (it already reloads externally-changed files via its file-presenter/iCloud
  coordination) — but **verify** the companion app picks up an okashi manifest write. Schema stays v1
  (unchanged), so this is an *authority* gate, not a *schema* gate.

---

## 1. Source model (mirror companion app §2)

The library is a **list of sources**, not one folder — exactly the companion app's model:

- **`sourcePrimary`** — today's `writingDir()` (`OKASHI_DIR`, else the iCloud `okashi` folder).
  Always present, never removable, the default home for new documents. (= companion app `.primary`.)
- **`sourceFolder(path)`** — user-added local/cloud folders (`~/Documents/writing`, a Dropbox
  folder = a synced local dir). (= companion app `.folder(path:)`.)
...
- **`sources.json`** in okashi's config dir (`os.UserConfigDir()/okashi/sources.json`) persists
  **user-added** sources only; the primary is synthesized at load (mirrors the companion app's
  `SourceStore`; the primary is never stored). Shape mirrors the companion app's `Source` (id/name/kind)
...
- **Per-source isolation:** an unreachable folder source (deleted/offline) is shown with an
  error affordance and skipped — never blocks the others or launch (companion app §8).
- **Standalone use (the TUI-only writer):** just add a folder source (or set `OKASHI_DIR`); no
  companion app or iCloud needed. A companion-app user has the shared corpus as one source among several.

## 2. Project model (align with companion app project-model §2)
...
- okashi **ignores the companion app's "project overlay"** (the marker/icon that groups manuscripts into a
  named Project) — the companion app itself says okashi treats a project as "a category-of-manuscripts it
...
- **`createManuscript`**: make `dir`, write `01-<slug>.md` (an empty first chapter) + a manifest
  ...
  JSON, atomic. (Matches the companion app's New-Project flow "container + first manuscript", adapted:
...
- Everything else (reorder/insert/move/convert) is NOT added — companion-app-owned.
...
NOT a 3rd column — the terminal can't spare the width; the companion app's Sources-column becomes a
...
- **Source picker (RESOLVED 2026-06-30 — cycle + inline add/remove):** the LIBRARY title shows the
  active source (e.g. `LIBRARY · Writing ▾`). **`s` cycles** the active source in place (skipping
...
1. **Source as a picker, not a column (lean).** The companion app has screen width for a Sources column;
...
2. **New Project makes a first chapter and opens it** — so `+ My Novel` lands you writing, not on
   an empty folder. (Matches the companion app's "container + first manuscript".)
3. **Pinning as a fast-follow** (the companion app's next slice): a top **Pinned** strip on home for
   deeply-nested projects/categories, app-side (`pins.json`), no core change. Leave room now.
4. **Multi-source search:** okashi already SHIPPED `ctrl+f` (the companion app defers search). Extend the
   engine to optionally search **all sources** (a scope beyond Project/Document) — okashi leads here.
5. **Keep `sources.json`/`pins.json` shapes aligned** with the companion app's `Source`/pin refs so the two
   apps stay conceptually identical (separate files, same fields).
6. **Don't build the project overlay** (icon/accent/named project container) — companion-app-only;
...
- Reorder / insert / move / convert (companion-app-owned, unchanged).
...
- contract: a manifest okashi writes is byte-shape-identical to the companion app's v1 (schemaVersion/title/
  items ordering) so the companion app reads it verbatim.

## 8. Sequencing (for the plan)

1. **Manifest writers** (`createManuscript`/`renameChapterTitle`/`writeManifest`) + the
   shared-contract doc updates in **BOTH** repos (okashi `CLAUDE.md` §1 **and** the companion app's
   `SPEC.md` / multi-source §4 / project-model §4 — reverse the committed "okashi never writes
   manifests; the only writers are the companion app's `ManuscriptStore`" line). No companion-app *code*
   change, but **verify** the companion app reloads an okashi-written manifest (§0). Wire "New Project"
   (folder+manifest+first chapter) + `r` retitle for manifest chapters — on **any** source
   incl. the companion app's shared corpus (create/retitle are no-confirm per §0), and **remove the
